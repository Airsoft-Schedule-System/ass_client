...
} from '@/features/games/game-session.format';
...
export function GameSessionCard({ isOwned, participationStatus, session }: GameSessionCardProps) {
  return (
    <Link
      ...
      className="flex w-full flex-col gap-4 rounded-2xl border border-[var(--color-app-border)] bg-[var(--color-app-surface)] p-4 transition-colors hover:border-[color-mix(in_srgb,var(--color-app-brand)_45%,transparent)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--color-app-brand)]"
      to={`/games/${session.id}`}
    >
      ...
      {participationStatus && (
        <div className="flex items-center justify-between gap-3">
          ...
          <GameStatusBadge status={participationStatus} />
        </div>
      )}
    </Link>
  );
